...
- **Highlights & notes** — select text to highlight it in one of five colours, attach a note, or start a comment thread with replies and **Resolve**; the toolbar carries a resolved count (which doubles as a show/hide switch) and an amber count of marks whose text has been edited away. Annotations are anchored to the words, not to a position, and stored beside the file in Application Support — keyed by path, so they survive an edit but not a rename — never written into your markdown
