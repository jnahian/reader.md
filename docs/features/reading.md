...
collapse, the toolbar goes, the window moves to fullscreen, and the canvas
narrows. Switch on **Dim other sections** in Settings and every section but
...
Dimming is off until you turn it on, and once on it follows the outline rather
than the scroll position, so it holds still while you read a section and fades
