...
- **`FileScanner` / `RootFolder`** — recursive markdown-only tree scan, pruning `node_modules`, `.git`, etc. In a repo it also prunes what `git ls-files --ignored` reports, matched as paths relative to the scanned folder — git answers in fully resolved paths while `FileManager` and the sidebar don't, so absolute keys from the two sides would never meet.
